...
func TestCreateQuota(t *testing.T) {
	client.API.(*mocks.Client).On("VolumePath", anyArgs[0:7]...).Return("").Once()
	client.API.(*mocks.Client).On("Post", anyArgs...).Return(nil).Once()
	_, err = client.CreateQuota(defaultCtx, quotaID, container, size, softLimit, advisoryLimit, softGracePrd, includeSnapshots)
	assert.Nil(t, err)
}

func TestCreateQuotaWithPath(t *testing.T) {
	client.API.(*mocks.Client).On("Post", anyArgs...).Return(nil).Once()
	_, err = client.CreateQuotaWithPath(defaultCtx, quotaID, container, size, softLimit, advisoryLimit, softGracePrd, includeSnapshots)
	assert.Nil(t, err)
}
